...
export function renderIncome(container, data, store) {
  ...
  const openAddModal = () => {
    openModal('수입 추가', `
      ${formRow(
      formField('date', '수입 일자', 'date', { required: true, value: today }),
      formField('period', '기록 년월', 'month', { required: true, value: thisMonth })
    )}
      ${formField('category', '수입 항목', 'select', { required: true, selectOptions: ['회사 급여', '상여금 & 기타 수당', '투자 수익', '프리랜서 수입', '기타 수입'] })}
      ${formField('amount', '수입 금액 (원)', 'number', { required: true, placeholder: '3000000', step: '1' })}
      ${formField('detail', '상세 내용', 'textarea', { placeholder: '수입에 대한 상세 내용을 입력하세요' })}
    `, (formData) => {
      store.addIncome({
        date: formData.date,
  ...
  container.innerHTML = `
    ...
    <div class="card animate-in animate-delay-2">
      <div class="card-header-row">
        <span class="card-title" style="margin-bottom:0">📋 수입 상세 내역</span>
      </div>
      ${items.length === 0 ? '<div class="empty-state"><div class="empty-state-icon">📭</div><p class="empty-state-text">수입 내역이 없습니다</p><p class="text-xs neutral mt-lg">위의 "수입 추가" 버튼을 눌러 수입을 기록하세요</p></div>' : `
